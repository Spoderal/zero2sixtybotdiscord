...
module.exports = {
    ...
    callback: (message, arguments, text, client) => {
        ...
        let alreadydis = db.fetch(`${commandtodisable.toLowerCase()}_disabled_${message.guild.id}`)

        if(!commandslist.includes(commandtodisable.toLowerCase())) return message.channel.send("Thats not a command!")
        if(!alreadydis) return message.channel.send("That command isn't disabled! If you'd like to disable it, run the disable command.")
        message.channel.send(`Disabled ${commandtodisable.toLowerCase()}`)

        db.set(`${commandtodisable.toLowerCase()}_disabled_${message.guild.id}`, false)
    },
    permissions: '',
    requiredRoles: [],
  }
